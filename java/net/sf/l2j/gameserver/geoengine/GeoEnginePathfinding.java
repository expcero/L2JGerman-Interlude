...
final class GeoEnginePathfinding extends GeoEngine
{
	...
	public List<Location> findPath(int ox, int oy, int oz, int tx, int ty, int tz, boolean playable)
	{
		...
		// Very long routes can allocate huge temporary buffers under load; reject them
		// safely and let movement fallback handle the situation.
		final int requiredSize = 64 + (2 * Math.max(Math.abs(gox - gtx), Math.abs(goy - gty)));
		NodeBuffer buffer = getBuffer(requiredSize, playable);
		if (buffer == null)
		{
			_findFails++;
			return null;
	...
	private final NodeBuffer getBuffer(int size, boolean playable)
	{
		for (BufferHolder holder : _buffers)
		{
			...
			if (holder == null || holder._size < size)
				continue;
			
			// Find unlocked preallocated NodeBuffer.
			...
			// No free buffer. Allocate only one temporary buffer of the first adequate size.
			// The previous loop could continue to larger holders and allocate extra huge buffers.
			final NodeBuffer temporary = new NodeBuffer(holder._size);
		...
		// Requested route is bigger than the largest configured buffer.
		return null;
	}
	...
	{
		List<String> list = new ArrayList<>(super.getStat());
