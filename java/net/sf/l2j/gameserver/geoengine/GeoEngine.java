...
public class GeoEngine
{
	...
	private final ConcurrentHashMap<Integer, ABlock[]> _regions;
	private final ConcurrentHashMap<Integer, Long> _regionLastAccess;
	private final Set<Integer> _geoRegions;
	private final BlockNull _nullBlock;
	private final AtomicInteger _regionLoadErrors;
	...
	public GeoEngine()
	{
		...
		// initialize lazy region cache
		_regions = new ConcurrentHashMap<>();
		_regionLastAccess = new ConcurrentHashMap<>();
		_geoRegions = ConcurrentHashMap.newKeySet();
		_regionLoadErrors = new AtomicInteger();
		
		// load null block
		_nullBlock = new BlockNull();
		
		// read geo files according to geoengine config setup, but don't load them yet
		...
		for (int rx = L2World.TILE_X_MIN; rx <= L2World.TILE_X_MAX; rx++)
		{
			for (int ry = L2World.TILE_Y_MIN; ry <= L2World.TILE_Y_MAX; ry++)
			{
				...
				{
					_geoRegions.add(getRegionKey(rx, ry));
					available++;
				}
			}
		}
		_log.info("GeoEngine: Registered " + available + " L2D region files for lazy loading.");
	...
	/**
	 * Loads one region on demand. Missing or broken regions are represented by null blocks.
	 * @param regionX : Geodata file region X coordinate.
	 * @param regionY : Geodata file region Y coordinate.
	 * @return {@link ABlock}[] : Region blocks.
	 */
	private final ABlock[] loadRegionBlocks(int regionX, int regionY)
	{
		final int regionKey = getRegionKey(regionX, regionY);
		final ABlock[] blocks = createNullRegionBlocks();
		
		if (!_geoRegions.contains(regionKey))
			return blocks;
		...
		if (!file.isFile())
		{
			_regionLoadErrors.incrementAndGet();
			_log.warning("GeoEngine: Registered region file " + filename + " doesn't exist. Using null blocks.");
			return blocks;
		}
		...
		{
			_regionLoadErrors.incrementAndGet();
			_log.warning("GeoEngine: Region file " + filename + " is empty. Using null blocks.");
			return blocks;
		}
		...
		try (RandomAccessFile raf = new RandomAccessFile(file, "r"); FileChannel fc = raf.getChannel())
		{
			...
			// BlockMultilayer uses a temporary static buffer, keep region parsing serialized.
			synchronized (BlockMultilayer.class)
			{
				...
				try
				{
					for (int ix = 0; ix < GeoStructure.REGION_BLOCKS_X; ix++)
					{
						for (int iy = 0; iy < GeoStructure.REGION_BLOCKS_Y; iy++)
						{
							...
							final int index = getRegionBlockIndex(ix, iy);
							
							switch (type)
							{
								case GeoStructure.TYPE_FLAT_L2D:
									blocks[index] = new BlockFlat(buffer, GeoFormat.L2D);
									break;
								
								case GeoStructure.TYPE_COMPLEX_L2D:
									blocks[index] = new BlockComplex(buffer, GeoFormat.L2D);
									break;
								
								case GeoStructure.TYPE_MULTILAYER_L2D:
									blocks[index] = new BlockMultilayer(buffer, GeoFormat.L2D);
									break;
			...
			_log.info("GeoEngine: Lazy loaded " + filename + " region file.");
			return blocks;
		}
		catch (Exception e)
		{
			_regionLoadErrors.incrementAndGet();
			_log.warning("GeoEngine: Error while lazy loading " + filename + " region file. Using null blocks for this region.");
			_log.warning(String.valueOf(e.getMessage()));
			return blocks;
		}
	}
	
	private final ABlock[] createNullRegionBlocks()
	{
		final ABlock[] blocks = new ABlock[GeoStructure.REGION_BLOCKS_X * GeoStructure.REGION_BLOCKS_Y];
		Arrays.fill(blocks, _nullBlock);
		return blocks;
	}
	
	private static final int getRegionKey(int regionX, int regionY)
	{
		return (regionX << 16) | (regionY & 0xFFFF);
	}
	
	private static final int getRegionBlockIndex(int blockX, int blockY)
	{
		return blockX * GeoStructure.REGION_BLOCKS_Y + blockY;
	}
	
	private final ABlock getBlockByBlockCoords(int blockX, int blockY)
	{
		if (blockX < 0 || blockY < 0 || blockX >= GeoStructure.GEO_BLOCKS_X || blockY >= GeoStructure.GEO_BLOCKS_Y)
			return _nullBlock;
		
		final int regionX = blockX / GeoStructure.REGION_BLOCKS_X + L2World.TILE_X_MIN;
		final int regionY = blockY / GeoStructure.REGION_BLOCKS_Y + L2World.TILE_Y_MIN;
		final int regionBlockX = blockX % GeoStructure.REGION_BLOCKS_X;
		final int regionBlockY = blockY % GeoStructure.REGION_BLOCKS_Y;
		final int regionKey = getRegionKey(regionX, regionY);
		
		_regionLastAccess.put(regionKey, System.currentTimeMillis());
		
		final ABlock[] blocks = _regions.computeIfAbsent(regionKey, key -> loadRegionBlocks(regionX, regionY));
		return blocks[getRegionBlockIndex(regionBlockX, regionBlockY)];
	}
	
	private final void setBlockByBlockCoords(int blockX, int blockY, ABlock block)
	{
		if (blockX < 0 || blockY < 0 || blockX >= GeoStructure.GEO_BLOCKS_X || blockY >= GeoStructure.GEO_BLOCKS_Y)
			return;
		...
		final ABlock[] blocks = _regions.computeIfAbsent(regionKey, key -> loadRegionBlocks(regionX, regionY));
		blocks[getRegionBlockIndex(regionBlockX, regionBlockY)] = block;
	}
	
	public final int getLoadedRegionCount()
	{
		return _regions.size();
	}
	
	public final int getAvailableRegionCount()
	{
		return _geoRegions.size();
	}
	
	public final int getRegionLoadErrorCount()
	{
		return _regionLoadErrors.get();
	}
	
	public final boolean isRegionLoaded(int regionX, int regionY)
	{
		return _regions.containsKey(getRegionKey(regionX, regionY));
	}
	
	public final boolean preloadRegion(int regionX, int regionY)
	{
		if (regionX < L2World.TILE_X_MIN || regionX > L2World.TILE_X_MAX || regionY < L2World.TILE_Y_MIN || regionY > L2World.TILE_Y_MAX)
			return false;
		
		final int key = getRegionKey(regionX, regionY);
		_regionLastAccess.put(key, System.currentTimeMillis());
		_regions.computeIfAbsent(key, k -> loadRegionBlocks(regionX, regionY));
		return true;
	}
	
	public final int unloadIdleRegions(long maxIdleMillis)
	{
		if (maxIdleMillis <= 0)
			return 0;
		...
		for (Integer key : _regions.keySet())
		{
			final Long lastAccess = _regionLastAccess.get(key);
			if (lastAccess == null || now - lastAccess < maxIdleMillis)
				continue;
			
			final ABlock[] blocks = _regions.get(key);
			if (blocks == null || containsDynamicBlock(blocks))
				continue;
			
			if (_regions.remove(key, blocks))
			{
				_regionLastAccess.remove(key);
				unloaded++;
			}
		}
	...
	private static final boolean containsDynamicBlock(ABlock[] blocks)
	{
	...
	{
		return getBlockByBlockCoords(geoX / GeoStructure.BLOCK_CELLS_X, geoY / GeoStructure.BLOCK_CELLS_Y);
	}
	...
	private final void toggleGeoObject(IGeoObject object, boolean add)
	{
		...
		for (int bx = minBX; bx <= maxBX; bx++)
		{
			...
			for (int by = minBY; by <= maxBY; by++)
			{
				...
				{
					// get related block
					block = getBlockByBlockCoords(bx, by);
					...
					if (!(block instanceof IBlockDynamic))
					{
						...
						if (block instanceof BlockFlat)
						{
							...
							setBlockByBlockCoords(bx, by, block);
						}
	...
	private static final long LOS_CACHE_CLEAN_INTERVAL = 30000;
	private static final int LOS_CACHE_MAX_SIZE = 4096;
	private static final int LOS_MIN_DISTANCE = 200;
	
	private final ConcurrentHashMap<Long, LosCache> _losCache = new ConcurrentHashMap<>();
	private volatile long _lastLosCacheClean = 0;
	...
	private static long getLosCacheKey(L2Object origin, L2Object target)
	{
		return (((long) origin.getObjectId()) << 32) ^ (target.getObjectId() & 0xffffffffL);
	}
	
	private void cleanupLosCache(long now)
	{
		if (now - _lastLosCacheClean < LOS_CACHE_CLEAN_INTERVAL && _losCache.size() <= LOS_CACHE_MAX_SIZE)
			return;
		
		_lastLosCacheClean = now;
		_losCache.entrySet().removeIf(entry -> entry.getValue() == null || now - entry.getValue().time > LOS_CACHE_TIME);
		
		// Hard safety guard: if many different origin/target pairs were generated,
		// prefer dropping the cache over retaining thousands of stale entries.
		if (_losCache.size() > LOS_CACHE_MAX_SIZE)
			_losCache.clear();
	}
	...
	public final boolean canSeeTarget(L2Object origin, L2Object target)
	{
		...
		if (!origin.isVisible() || !target.isVisible())
			return false;
		...
		// verifica cache por par origin/target. O cache antigo usava apenas targetId,
		// o que podia reutilizar resultado de outro origin e ainda acumular IDs antigos.
		final long cacheKey = getLosCacheKey(origin, target);
		LosCache cache = _losCache.get(cacheKey);
		if (cache != null && now - cache.time < LOS_CACHE_TIME)
		{
			if (cache.ox == ox && cache.oy == oy && cache.oz == oz && cache.tx == tx && cache.ty == ty && cache.tz == tz)
				return cache.result;
		}
		...
		_losCache.put(cacheKey, newCache);
	...
	public List<String> getStat()
	{
		...
		stats.add("Loaded geodata regions: " + _regions.size() + " / " + _geoRegions.size());
		stats.add("Lazy geodata cache blocks: " + (_regions.size() * GeoStructure.REGION_BLOCKS_X * GeoStructure.REGION_BLOCKS_Y));
		stats.add("Geodata lazy load errors: " + _regionLoadErrors.get());
		stats.add("LOS cache entries: " + _losCache.size() + " / " + LOS_CACHE_MAX_SIZE);
		return stats;
	}
