...
public class AutofarmPlayerRoutine
{
	...
	private volatile boolean waitingForSweep = false; // Flag indicando se está aguardando sweep
	...
	{
		return !stopped && player != null && player.isOnline() && player.isAutoFarm();
	}
	...
	private synchronized void cancelSweepTask()
	{
		if (sweepTask != null)
		{
			if (!sweepTask.isDone())
	...
	public void useItem(ItemInstance item)
	{
		...
		if (item.isEquipable())
		{
			...
			if (player.isAttackingNow())
				...
				}, Math.max(0, player.getAttackEndTime() - System.currentTimeMillis()));
			else
